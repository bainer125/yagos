	...
	constructor ( id , callback ){
		...
		this.delete_penalties = function (){
			...
			this.home_penalties.forEach(function(item,index){
				...
				if (item.start + item.finish < this.elap){
		...
		this.add_penalty = function ( team , pen ){
			if (team.trim().toLowerCase() == 'home'){
				this.home_penalties.push(pen);
			}
			else if (team.trim().toLowerCase() == 'away'){
				this.away_penalties.push(pen);
			}
		}
